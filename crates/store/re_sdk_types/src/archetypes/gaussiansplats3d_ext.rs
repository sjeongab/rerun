...
impl GaussianSplats3D {
    /// Creates a new [`GaussianSplats3D`] from a `.ply` file.
    ///
    ...
    /// Creates a new [`GaussianSplats3D`] from the contents of a `.ply` file.
    ///
...
fn from_ply(ply: ply_rs_bw::ply::Ply<ply_rs_bw::ply::DefaultElement>) -> GaussianSplats3D {
    re_tracing::profile_function!();
    ...
    let mut arch = crate::archetypes::GaussianSplats3D::new(positions);
