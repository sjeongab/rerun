...
// ---
pub struct GaussianSplats3DVisualizer(SpatialViewVisualizerData);
...
impl GaussianSplats3DVisualizer {
    fn process_data<'a>(
        ...
        batches: impl Iterator<Item = GaussianSplats3DComponentData<'a>>,
        ...
        for batch in batches {
            ...
            builder.add_batch(
                ...
                GaussianSplats3D::descriptor_show_labels().component,
    ...
    fn execute(
        ...
        process_archetype::<Self, GaussianSplats3D, _>(
            ...
            |ctx, spatial_ctx, results| {
                let all_half_sizes =
                    results.iter_required(GaussianSplats3D::descriptor_half_sizes().component);
                if all_half_sizes.is_empty() {
                    return Ok(());
                }

                // TODO(andreas): Introduce a utility for this?
                let num_gaussiansplats: usize = all_half_sizes
                    .chunks()
                    .iter()
                    .flat_map(|chunk| chunk.iter_slices::<[f32; 3]>())
                    .map(|vectors| vectors.len())
                    .sum();
                if num_gaussiansplats == 0 {
                    return Ok(());
                }
                ...
                let all_centers =
                    results.iter_optional(GaussianSplats3D::descriptor_centers().component);
                let all_rotation_axis_angles = results
                    .iter_optional(GaussianSplats3D::descriptor_rotation_axis_angles().component);
                let all_quaternions =
                    results.iter_optional(GaussianSplats3D::descriptor_quaternions().component);
                let all_colors = results.iter_optional(GaussianSplats3D::descriptor_colors().component);
                let all_line_radii =
                    results.iter_optional(GaussianSplats3D::descriptor_line_radii().component);
                let all_fill_modes =
                    results.iter_optional(GaussianSplats3D::descriptor_fill_mode().component);
                let all_labels = results.iter_optional(GaussianSplats3D::descriptor_labels().component);
                let all_class_ids =
                    results.iter_optional(GaussianSplats3D::descriptor_class_ids().component);
                let all_show_labels =
                    results.iter_optional(GaussianSplats3D::descriptor_show_labels().component);
